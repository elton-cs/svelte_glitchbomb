<script lang="ts">
  ...
  function formatLogEntry(entry: GameLogEntry): string {
    ...
    if (action.includes('Converted') && action.includes('points to') && action.includes('bits')) {
      // Extract bits amount from "Converted X points to Y bits (+Y total: Z)"
      const bitsMatch = action.match(/Converted \d+ points to (\d+) bits/);
      if (bitsMatch) {
        const bits = bitsMatch[1];
        return `<span class="text-blue-500">BITS</span> > +${bits} B`;
      } else {
        return `<span class="text-blue-500">BITS</span> > ${action} ${details}`.trim();
      }
      ...
      // Extract item name and cost from "Bought [item] for [cost] bits"
      const shopMatch = action.match(/Bought (.+?) for (\d+) bits/);
      if (shopMatch) {
        const item = shopMatch[1];
        const cost = shopMatch[2];
        return `<span class="text-orange-400">MOD SHOP</span> > install ${item} > -${cost} B`;
      } else {
      ...
      if (levelMatch) {
        ...
        return `<span class="text-purple-500">ADVANCE</span> > level ${level} > -${cost} GB`;
      } else {
      ...
      if (gbMatch) {
        const gb = gbMatch[1];
        return `<span class="text-cyan-500">GLITCH BYTES</span> > +${gb} GB`;
      } else {
        return `<span class="text-cyan-500">GLITCH BYTES</span> > ${action} ${details}`.trim();
      }
    } else if (action.includes('bits')) {
      const match = action.match(/(\d+) bits/);
      const bits = match ? match[1] : '';
      return `<span class="text-yellow-400">SPECIAL</span> > +${bits} Bits`;
    } else if (action.includes('glitchbytes')) {
      const match = action.match(/(\d+) glitchbytes/);
      const bytes = match ? match[1] : '';
      return `<span class="text-yellow-400">SPECIAL</span> > +${bytes} Glitch Bytes`;
