...
export function startNewGame(gameState: GameState): boolean {
  try {
    // Preserve current glitchbytes before resetting
    const currentGlitchbytes = gameState.playerStats.glitchbytes;
    
    // Reset entire game state to initial state (clears all session data)
    ...
    // Enter level 1 (this will change phase from 'menu' to 'level')
...
export function cashOutMidLevel(gameState: GameState): number {
  ...
  // Reset entire game state to initial state, preserving updated glitchbytes
...
export function returnToMenu(gameState: GameState): void {
  ...
  // Reset entire game state to initial state, preserving glitchbytes
  const newState = createInitialGameState(currentGlitchbytes);
  Object.assign(gameState, newState);
}
