<script lang="ts">
  ...
  function get_category_stats(category: OrbCategory) {
    if (!game || !game.playground_orbs || game.playground_orbs.length === 0) {
      return { count: 0, percentage: 0 };
    }

    const total_orbs = game.playground_orbs.length;
    const count = game.playground_orbs.filter(orb => orb.category === category).length;
    const percentage = total_orbs > 0 ? Math.round((count / total_orbs) * 100) : 0;
  ...
  {:else}
    ...
    <div class="grid grid-cols-5 gap-2">
      {#each [OrbCategory.Bomb, OrbCategory.Health, OrbCategory.Point, OrbCategory.Multiplier, OrbCategory.Special] as category}
        ...
        <div class="{info.color} border border-white rounded p-2 text-center">
          <div class="text-black text-xs font-bold">{info.initial}</div>
