<script lang="ts">
  ...
  import OrbTypeDisplay from "../../components/OrbTypeDisplay.svelte";
  import ChipIcon from "../comps/ChipIcon.svelte";
    ...
    switch (type) {
      case "point":
        const multipliedPoints = Math.floor(
          amount * gameState.playerStats.levelMultiplier
        );
        return gameState.playerStats.levelMultiplier > 1
          ? `${amount}×${gameState.playerStats.levelMultiplier} = ${multipliedPoints} pts`
          : `${amount} pts`;
      case "points_per_anyorb":
        const remaining = totalAvailableOrbs - 1;
        const comboPoints = Math.floor(
          amount * remaining * gameState.playerStats.levelMultiplier
        );
      ...
      case "points_per_bombpulled":
        const dangerPoints = Math.floor(
          amount *
            gameState.playerStats.bombsPulledThisLevel *
            gameState.playerStats.levelMultiplier
        );
        if (gameState.playerStats.levelMultiplier > 1) {
  ...
  const pointsPercentage = $derived(
    totalAvailableOrbs > 0
      ? Math.round((totalPointsAvailable / totalAvailableOrbs) * 100)
      : 0
  );

  const specialPercentage = $derived(
    totalAvailableOrbs > 0
      ? Math.round((totalSpecialAvailable / totalAvailableOrbs) * 100)
      : 0
  );
</script>
...
>
  <div class="flex justify-between items-center mb-3">
    ...
    <div class="text-xs text-white font-mono min-h-4">
      {calculationDisplay || "hover for details"}
    </div>
  </div>
  ...
  <div class="grid grid-cols-5 gap-2 text-sm flex-1 overflow-y-auto">
    ...
    <div class="space-y-2 {totalPointsAvailable === 0 ? 'opacity-50' : ''}">
      ...
      <div class="text-center mb-2">
        ...
        <h3
          class="font-medium {totalPointsOwned === 0
            ? 'text-gray-500'
        ...
        <span
          class="{totalPointsOwned === 0
            ? 'text-gray-500'
            : 'text-green-400'} text-sm"
          >{totalPointsAvailable}/{totalPointsOwned}</span
        >
        <div
          class="{totalPointsOwned === 0
            ? 'text-gray-500'
          ...
          {#each pointsOrbTypes as pointType}
            {#each gameState.orbBag[pointType.type].available as orb}
              <div class="relative group">
                <div
                  ...
                  tabindex="0"
                  onmouseenter={() =>
                    handleOrbHover(pointType.type, orb.amount)}
            ...
            {#if gameState.orbBag[pointType.type].total.length > gameState.orbBag[pointType.type].available.length}
              {#each Array(gameState.orbBag[pointType.type].total.length - gameState.orbBag[pointType.type].available.length) as _, consumedIndex}
                {@const consumedOrb =
                  gameState.orbBag[pointType.type].total[
                    gameState.orbBag[pointType.type].available.length +
                      consumedIndex
                  ]}
                <div
                  class="min-w-8 h-8 px-1 border border-gray-600 bg-gray-800 text-green-600 flex items-center justify-center text-sm"
    ...
    <div class="space-y-2 {totalSpecialAvailable === 0 ? 'opacity-50' : ''}">
      ...
      <div class="text-center mb-2">
        ...
        <h3
          class="font-medium {totalSpecialOwned === 0
            ? 'text-gray-500'
        ...
        <span
          class="{totalSpecialOwned === 0
            ? 'text-gray-500'
            : 'text-yellow-400'} text-sm"
          >{totalSpecialAvailable}/{totalSpecialOwned}</span
        >
        <div
          class="{totalSpecialOwned === 0
            ? 'text-gray-500'
          ...
          {#each specialOrbTypes as specialType}
            {#each gameState.orbBag[specialType.type].available as orb}
              <div class="relative group">
                <div
                  ...
                  tabindex="0"
                  onmouseenter={() =>
                    handleOrbHover(specialType.type, orb.amount)}
                  onmouseleave={handleOrbLeave}
            ...
            {#if gameState.orbBag[specialType.type].total.length > gameState.orbBag[specialType.type].available.length}
              {#each Array(gameState.orbBag[specialType.type].total.length - gameState.orbBag[specialType.type].available.length) as _, consumedIndex}
                {@const consumedOrb =
                  gameState.orbBag[specialType.type].total[
                    gameState.orbBag[specialType.type].available.length +
                      consumedIndex
                  ]}
                <div
                  class="min-w-8 h-8 px-1 border border-gray-600 bg-gray-800 text-yellow-600 flex items-center justify-center text-sm"
