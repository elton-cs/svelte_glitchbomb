...
<div
  class="p-1 rounded-lg shadow-sm border border-white h-full flex flex-col {(gameState.phase ===
    'marketplace' ||
    ...
    {#each shopInventory as item}
      ...
      >
        ...
        {#if item.purchaseCount > 0}
          <div
            class="absolute bottom-0.5 left-0.5 text-[8px] font-bold px-1 py-0.5 rounded-full min-w-4 text-center z-10 border transition-colors
                      {item.available &&
            item.canPurchase &&
            gameState.phase === 'marketplace' &&
            gameState.marketplace.available
              ? 'text-white border-white group-hover:bg-white group-hover:text-black group-hover:border-black'
                  ...
                  <div class="flex-1 flex items-center justify-center mb-1">
                    <div
                      ...
                      gameState.marketplace.available
                        ...
                        : 'border-gray-600'} rounded px-2 py-1 flex flex-col items-center justify-center min-h-12 min-w-12 transition-colors
                                {item.available &&
                      item.canPurchase &&
                      gameState.phase === 'marketplace' &&
                      gameState.marketplace.available
                        ...
                        : ''}"
                    >
                      <div class="text-lg mb-0.5">{item.orbDisplay.icon}</div>
                      {#if item.orbDisplay.isChipOrb}
                        <div class="flex items-center gap-0.5">
                          <div
                            class="text-xs font-bold {item.available &&
                            item.canPurchase &&
                            gameState.phase === 'marketplace' &&
                            gameState.marketplace.available
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-0.5">
                  {#if item.available && item.cost > 0}
                    {#if item.purchaseCount > 0}
                      <div class="text-[10px] opacity-60 line-through">
                        {item.baseCost}
                      </div>
                    {/if}
                    <div
